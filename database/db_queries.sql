...
SELECT poll_option.name, count(*) as n_votes
	FROM poll_option, vote_on_poll, users
	WHERE poll_option.id_poll= $poll_id
		AND poll_option.id_poll_option=vote_on_poll.id_poll_option
		AND vote_on_poll.id_user = users.id_user
		AND users.active=true
	GROUP BY poll_option.name
